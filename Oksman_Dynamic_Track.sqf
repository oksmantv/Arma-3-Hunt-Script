				...
				sleep 5;
					...
					if(oks_debug) then {SystemChat format ["Nearest Player: %1  Hunter: %2",_nearestPlayer,_Hunter]; };
						...
						_wp setWaypointSpeed "NORMAL";
							...
							if (vehicle _hunter isKindOf "LandVehicle") then
							{
								_null = [group _Hunter, position _nearestPlayer, 700] call bis_fnc_taskPatrol;
							};
							if (vehicle _hunter isKindOf "Air") then
							{
								_null = [group _Hunter, position _nearestPlayer, 1500] call bis_fnc_taskPatrol;
							};
							if (vehicle _hunter isKindOf "Man") then
							{
								_null = [group _Hunter, position _nearestPlayer, 400] call bis_fnc_taskPatrol;
							};
